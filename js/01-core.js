...
function toRow(l){const r={id:l.id,tipo:l.tipo,data_comp:l.dataComp||null,data_venc:effectiveVenc(l)||null,data_pgto:l.dataPgto||null,cat:l.cat,sub:l.sub||null,descricao:l.desc||null,cc:l.cc||null,cliente_id:l.clienteId||null,forma:l.forma||null,conta:normalizeConta(l.conta)||null,doc:l.doc||null,valor_bruto:parseMoney(l.valorBruto),ded:parseMoney(l.ded),valor_liq:parseMoney(l.valorLiq),status:l.status,obs:l.obs||null,recorrente:!!l.recorrente};if(l.parentId)r.parent_id=l.parentId;if(l.adjType)r.adj_type=l.adjType;return r;}
function fromRow(r){const status=r.status||'Pendente';const legacyVenc=!r.data_venc&&(status==='Pendente'||status==='Parcial')?r.data_pgto||'':'';const dataVenc=r.data_venc||legacyVenc;const dataPgto=(status==='Pago'||status==='Recebido'||status==='Parcial')?r.data_pgto||'':'';const recorrente=r.recorrente===true?true:(r.recorrente==null&&typeof r.obs==='string'&&r.obs.includes('[recorrente]'));return{id:r.id,seq:r.seq||null,tipo:r.tipo,dataComp:r.data_comp||'',dataVenc,dataPgto,cat:r.cat||'',sub:r.sub||'',desc:r.descricao||'',cc:r.cc||'',clienteId:r.cliente_id||'',forma:r.forma||'PIX',conta:r.conta||'',doc:r.doc||'',valorBruto:r.valor_bruto||0,ded:r.ded||0,valorLiq:r.valor_liq||0,status,obs:r.obs||'',recorrente,parentId:r.parent_id||null,adjType:r.adj_type||null};}
